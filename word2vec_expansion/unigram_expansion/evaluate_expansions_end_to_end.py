...
def _normalize_unigram_set(terms: List[str]) -> Set[str]:
    ...
    for t in terms:
        ...
        if len(toks) == 1:
            ...
            dropped += 1
    if dropped:
        print(f"[warn] dropped {dropped} ground-truth terms that normalized to multi-token; unigram-only evaluation.")
    return out
...
def expand_seeds_exact(model: Word2Vec, seed_json: str, topk: int, min_cos: Optional[float]):
    ...
    if isinstance(seeds_obj, dict):
        ...
        raw_seeds = [t for t, _ in pairs]
        seeds = [" ".join(process_text(t)) for t in raw_seeds]
        seed_to_cat = {}
        for (t, c), s_norm in zip(pairs, seeds):
            if s_norm:
                seed_to_cat[s_norm] = c
    else:
        raw_seeds = list(seeds_obj)
        seeds = [" ".join(process_text(t)) for t in raw_seeds]
...
def main():
    ...
    p.add_argument("--min_cos", type=float, default=None, help="Min cosine; strict '>' filter after topk")
    ...
    # 4) Evaluate (EXACT)
    results_eval = evaluate_on_docs_exact(docs_raw, det_terms, gt_terms)
