...
def build_exact_path(path, arch, k):
    """
    Build an exact path for the output file based on the provided path and k.
    """
    path = Path(path)
    if args.min_cos is None:
        return path / f"topk_{k}.json"
    return path / f"topk_{k}_min_cos_{args.min_cos}_{arch}.json"
...
def main(args):
    ...
    if args.exact_path:
        out_path = build_exact_path(args.exact_path, arch, args.topk)
    else:
...
if __name__ == "__main__":
    ...
    p.add_argument("--seed_json",  default='vocabulary_evaluation/manual_terms_7_12/all_terms/1_to_3_gram_seed_terms.json',
    ...
    p.add_argument("--exact_path", type=str, default=None,)
    args = p.parse_args()
    main(args)
